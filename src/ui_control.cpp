...
void update_resolution(GtkWidget* widget) {
  ...
  g_signal_handlers_block_by_func(GTK_COMBO_BOX(combo_box_frame_rate),
  ...
  gtk_combo_box_text_append_text(GTK_COMBO_BOX_TEXT(combo_box_frame_rate),
                                 cur_fps);
...
void menu_item_formater(GtkWidget* item, GtkWidget* menu, GCallback handler) {
...
void open_config_dialog(GtkWidget* widget, gpointer window) {
  ...
  GtkWidget* file_dialog = gtk_file_chooser_dialog_new(
      "Load Config File", GTK_WINDOW(window), GTK_FILE_CHOOSER_ACTION_OPEN,
      ("_Open"), GTK_RESPONSE_OK, ("_Cancel"), GTK_RESPONSE_CANCEL, NULL);
...
void fw_update_clicked(GtkWidget* item) {
  if (strcmp(gtk_menu_item_get_label(GTK_MENU_ITEM(item)), "Erase Firmware") ==
      0) {
...
void exit_from_help(GtkWidget* widget) { exit_loop(widget); }
...
void register_write() {
  ...
  if (gtk_toggle_button_get_active(
    ...
    int regAddr =
        entry_formater(entry_reg_addr, "register address", 'x', 0xffff, 0x0);
    if ((address_width_flag) == _8BIT_FLG) regAddr = regAddr & 0xff;

    int regVal =
        entry_formater(entry_reg_val, "register value", 'x', 0xffff, 0x00);
    if ((value_width_flag) == _8BIT_FLG) regVal = regVal & 0xff;
  ...
  /** generic i2c slave write */
    ...
    int slaveAddr =
        entry_formater(entry_i2c_addr, "slave address", 'x', 0xff, 0x1);
    int regAddr =
        entry_formater(entry_reg_addr, "register address", 'x', 0xffff, 0x0);
    if ((address_width_flag) == _8BIT_FLG) regAddr = regAddr & 0xff;
    int regVal =
        entry_formater(entry_reg_val, "register value", 'x', 0xffff, 0x0);
    if ((value_width_flag) == _8BIT_FLG) regVal = regVal & 0xff;
    unsigned char buf[2];
    buf[0] = regVal & 0xff;
    buf[1] = (regVal >> 8) & 0xff;
    ...
    generic_I2C_write(v4l2_dev, (GENERIC_REG_WRITE_FLG | address_width_flag),
                      value_width_flag, slaveAddr, regAddr, buf);
  }
}
...
void register_read() {
  ...
  /** generic i2c slave read */
    ...
    int regVal =
        generic_I2C_read(v4l2_dev, GENERIC_REG_READ_FLG | address_width_flag,
                         value_width_flag, slaveAddr, regAddr);
...
void black_level_correction() {
  int blc = entry_formater(entry_blc, "black level correction", 'd', 512, -512);
  add_blc(blc);
}
...
void set_rgb_gain_offset() {
  int red_gain = entry_formater(entry_red_gain, "red gain", 'd', MAX_RGB_GAIN,
                                MIN_RGB_GAIN);
  int green_gain = entry_formater(entry_green_gain, "green gain", 'd',
                                  MAX_RGB_GAIN, MIN_RGB_GAIN);
  int blue_gain = entry_formater(entry_blue_gain, "blue gain", 'd',
                                 MAX_RGB_GAIN, MIN_RGB_GAIN);
  int red_offset = entry_formater(entry_red_offset, "red offset", 'd',
                                  MAX_RGB_OFFSET, MIN_RGB_OFFSET);
  int green_offset = entry_formater(entry_green_offset, "green offset", 'd',
                                    MAX_RGB_OFFSET, MIN_RGB_OFFSET);
  int blue_offset = entry_formater(entry_blue_offset, "blue offset", 'd',
                                   MAX_RGB_OFFSET, MIN_RGB_OFFSET);
...
void set_rgb_matrix() {
  int red_red =
      entry_formater(entry_rr, "red red", 'd', MAX_RGB_MATRIX, MIN_RGB_MATRIX);
  int red_green = entry_formater(entry_rg, "red green", 'd', MAX_RGB_MATRIX,
                                 MIN_RGB_MATRIX);
  int red_blue =
      entry_formater(entry_rb, "red blue", 'd', MAX_RGB_MATRIX, MIN_RGB_MATRIX);
  int green_red = entry_formater(entry_gr, "green red", 'd', MAX_RGB_MATRIX,
                                 MIN_RGB_MATRIX);
  int green_green = entry_formater(entry_gg, "green green", 'd', MAX_RGB_MATRIX,
                                   MIN_RGB_MATRIX);
  int green_blue = entry_formater(entry_gb, "green blue", 'd', MAX_RGB_MATRIX,
                                  MIN_RGB_MATRIX);
  int blue_red =
      entry_formater(entry_br, "blue red", 'd', MAX_RGB_MATRIX, MIN_RGB_MATRIX);
  int blue_green = entry_formater(entry_bg, "blue green", 'd', MAX_RGB_MATRIX,
                                  MIN_RGB_MATRIX);
  int blue_blue = entry_formater(entry_bb, "blue blue", 'd', MAX_RGB_MATRIX,
                                 MIN_RGB_MATRIX);

  if (gtk_toggle_button_get_active(
          GTK_TOGGLE_BUTTON(check_button_rgb_matrix))) {
    enable_rgb_matrix(red_red, red_green, red_blue, green_red, green_green,
                      green_blue, blue_red, blue_green, blue_blue);
  } else
    disable_rgb_matrix();
}
...
void enable_soft_ae(GtkToggleButton* toggle_button) {
  enable_flg_formater(toggle_button, "Software AE", &soft_ae_enable);
...
void enable_show_edge(GtkToggleButton* toggle_button) {
  enable_flg_formater(toggle_button, "Show canny edge", &canny_filter_enable);
  if (gtk_toggle_button_get_active(toggle_button))
...
void ov580_register_write() {
  if ((ov580_dev_flag) == _OV580_FLG) {
    int regAddr = entry_formater(entry_ov580_reg_addr, "register address", 'x',
                                 0xffff, 0x0);
    unsigned char regVal =
        entry_formater(entry_ov580_reg_val, "register value", 'x', 0xff, 0x0);

    ov580_write_system_reg(v4l2_dev, regAddr, regVal);

  } else {
    unsigned char slaveAddr =
        entry_formater(entry_ov580_i2c_addr, "slave address", 'x', 0xff, 0x1);
    int regAddr = entry_formater(entry_ov580_reg_addr, "register address", 'x',
                                 0xffff, 0x0);
    unsigned char regVal =
        entry_formater(entry_ov580_reg_val, "register value", 'x', 0xff, 0x00);
...
void init_grid1_widgets() {
  ...
  if (exposure_max == 0) exposure_max = 999;

  if (gain_max == 0) gain_max = 13;
  ...
  if (gain_max == 13) gtk_widget_set_sensitive(hscale_gain, 0);
  if (exposure_max == 999) gtk_widget_set_sensitive(hscale_exposure, 0);
}
/**
...
void iterate_def_elements(def_element* definitions, size_t members) {
  FOREACH_NELEM(definitions, members, def) {
    switch (def->wid_type) {
      ...
      case GTK_WIDGET_TYPE_COMBO_BOX:
        gtk_combo_box_text_append_text(GTK_COMBO_BOX_TEXT(def->widget),
                                       def->label_str);
...
void init_grid3_def_elements() {
  ...
  for (int i = 1; i < resolution_counts; i++) {
    gtk_combo_box_text_append_text(GTK_COMBO_BOX_TEXT(combo_box_resolution),
                                   resolutions[i].c_str());
  }
  ...
  for (res_index = 0; res_index < resolution_counts; res_index++) {
    std::vector<std::string> elem = split(resolutions[res_index], 'x');

    int height = stoi(elem[1]);
    if (get_current_height(v4l2_dev) == height) {
      //printf("res index = %d\r\n", res_index);
      gtk_combo_box_set_active(GTK_COMBO_BOX(combo_box_resolution), res_index);
    }
  }
}

/** iterate over grid element for GUI layout */
void iterate_grid_elements(GtkWidget* grid, grid_elements* elements,
                           size_t members) {
  FOREACH_NELEM(elements, members, ele) {
    g_assert(ele->widget != NULL);
    gtk_grid_attach(GTK_GRID(grid), ele->widget, ele->col, ele->row, ele->width,
                    1);
  }
}
...
void iterate_window_signals(GtkWidget* widget, window_signal* signals,
                            size_t members) {
...
void init_sensitivity() {
  ...
  gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(check_button_stream_info),
                               true);
  gtk_range_set_value(GTK_RANGE(hscale_beta), 0);
  gtk_widget_set_sensitive(hscale_edge_low_thres, 0);
}
...
void notebook_setup(GtkWidget* maintable) {
  ...
  GtkWidget* icon_image =
      gtk_image_new_from_icon_name("camera-photo", GTK_ICON_SIZE_LARGE_TOOLBAR);
  ...
  GtkWidget* icon_video = gtk_image_new_from_icon_name(
      "video-display", GTK_ICON_SIZE_LARGE_TOOLBAR);
  ...
  //////////////////////////////////////////////////////
  GtkWidget* icon_ctrl =
      gtk_image_new_from_icon_name("media-flash", GTK_ICON_SIZE_LARGE_TOOLBAR);
...
void menu_bar_setup(GtkWidget* maintable) {
  ...
  /** device info items */
  GtkWidget* device_item = gtk_menu_item_new_with_label("Devices Info");
  ...
  device_info.push_back("Device: " + std::string(get_dev_name()));
  device_info.push_back("Manufacturer: " +
                        std::string(get_manufacturer_name()));
  device_info.push_back("Product: " + std::string(get_product()));

  if (!is_ov580_stereo()) {
    device_info.push_back("Firmware Rev: " + std::to_string(get_fw_rev()));
    char buf[10];
    snprintf(buf, sizeof(buf), "0%x", get_hw_rev());
    device_info.push_back("Hardware Rev: " + std::string(buf));
    ...
    if ((strcmp(get_product(), "USB Camera-OV580") == 0) ||
      ...
      bpp = YUYV_FLG;
    device_info.push_back("Datatype: " + convert_bpp_to_string(bpp));
  }

  for (size_t i = 0; i < device_info.size(); i++) {
    device_item = gtk_menu_item_new_with_label(device_info[i].c_str());
  ...
  if (!is_ov580_stereo()) {
    ...
    menu_item_formater(config_file_item, file_menu,
    ...
    menu_item_formater(fw_update_item, fw_update_menu,
  ...
  menu_item_formater(help_item, help_menu, G_CALLBACK(about_info));

  help_item = gtk_menu_item_new_with_label("Exit");
  menu_item_formater(help_item, help_menu, G_CALLBACK(exit_loop));
...
void css_setup() {
  ...
  if (g_file_test("../gui_style.css", G_FILE_TEST_EXISTS)) {
    gtk_css_provider_load_from_file(
        provider, g_file_new_for_path("../gui_style.css"), NULL);
  } else {
    gtk_css_provider_load_from_file(
        provider, g_file_new_for_path("./gui_style.css"), NULL);
  }
