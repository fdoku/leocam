...
void set_restart_flag(int flag) { *restart = flag; }
 ...
 */
void video_capture_save_raw() { set_save_raw_flag(1); }
 ...
 */
inline void set_save_raw_flag(int flag) { *save_raw = flag; }
...
int v4l2_core_save_data_to_file(const void* data, int size) {
  ...
  int ret = 0;
  char buf_name[40];
  snprintf(buf_name, sizeof(buf_name), "captures_%s_%d.raw", get_product(),
           image_count);
  ...
  if ((fp = fopen(buf_name, "wb")) != NULL) {
    ...
    if (fsync(fileno(fp)) || fclose(fp))
      fprintf(stderr,
              "V4L2_CORE: (save_data_to_file) error \
 ...
 */
void video_capture_save_bmp() { set_save_bmp_flag(1); }
 ...
 * 			   reset flag to zero once image is saved
 */
inline void set_save_bmp_flag(int flag) { *save_bmp = flag; }
...
static int save_frame_image_bmp(cv::InputOutputArray& opencvImage) {
  ...
  cv::imwrite(cv::format("captures_%s_%d.bmp", get_product(), image_count),
              opencvImage);
  image_count++;
  set_save_bmp_flag(0);
  return 0;
...
void change_datatype(void* datatype) {
  if (strcmp((char*)datatype, "1") == 0) *bpp = RAW10_FLG;
  if (strcmp((char*)datatype, "2") == 0) *bpp = RAW12_FLG;
  if (strcmp((char*)datatype, "3") == 0) *bpp = YUYV_FLG;
  if (strcmp((char*)datatype, "4") == 0) *bpp = RAW8_FLG;
  if (strcmp((char*)datatype, "5") == 0) *bpp = MJPEG_FLG;
}
...
void change_bayerpattern(void* bayer) {
  if (strcmp((char*)bayer, "1") == 0) *bayer_flag = CV_BayerBG2BGR_FLG;
  if (strcmp((char*)bayer, "2") == 0) *bayer_flag = CV_BayerGB2BGR_FLG;
  if (strcmp((char*)bayer, "3") == 0) *bayer_flag = CV_BayerRG2BGR_FLG;
  if (strcmp((char*)bayer, "4") == 0) *bayer_flag = CV_BayerGR2BGR_FLG;
  if (strcmp((char*)bayer, "5") == 0) *bayer_flag = CV_MONO_FLG;
}

/** callback for set blc from gui */
void add_blc(int blc_val_from_gui) { *blc = blc_val_from_gui; }
 ...
 */
void awb_enable(int enable) { enable_wrapper(awb_flag, enable); }
 ...
 * 		enable - set/reset the flag when get check button toggle
 */
void abc_enable(int enable) { enable_wrapper(abc_flag, enable); }
 ...
 * 		enable - set/reset the flag when get check button toggle
 */
void clahe_enable(int enable) { enable_wrapper(clahe_flag, enable); }
...
void* mmap_wrapper(int len) {
  void* p = mmap(NULL, len, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_ANONYMOUS,
                 -1, 0);
  if (p == (void*)-1) perror("mmap shared memory failed\n");
  return p;
}
...
void unmap_wrapper(T* data) {
  ...
  if (ret < 0) printf("Unable to unmap shared memory (%d)\n", errno);
}
 ...
 *
 */
void video_set_format(struct device* dev, int width, int height,
                      int pixelformat) {
  ...
  printf(
      "Get Video Format: %c%c%c%c (%08x) %ux%u\n"
      "byte per line:%d\nsize image:%u\n",
      (fmt.fmt.pix.pixelformat >> 0) & 0xff,
      (fmt.fmt.pix.pixelformat >> 8) & 0xff,
      (fmt.fmt.pix.pixelformat >> 16) & 0xff,
      (fmt.fmt.pix.pixelformat >> 24) & 0xff, fmt.fmt.pix.pixelformat,
...
std::vector<std::string> get_resolutions(struct device* dev) {
  ...
  while (ioctl(dev->fd, VIDIOC_ENUM_FMT, &fmtdesc) == 0) {
    ...
    for (frms.index = 0; !ioctl(dev->fd, VIDIOC_ENUM_FRAMESIZES, &frms);
         frms.index++) {
      if (frms.type == V4L2_FRMSIZE_TYPE_DISCRETE) {
        std::string each_line = std::to_string(frms.discrete.width) + "x" +
                                std::to_string(frms.discrete.height);
...
std::vector<int> get_frame_rates(struct device* dev) {
 ...
 * 		flag - set/reset the flag when get check button toggle
 */
void soft_ae_enable(int enable) { enable_wrapper(soft_ae_flag, enable); }
/**
  ...
  if (bytes_per_pixel == 2) {
    for (int i = 0; i < size; i++) {
      for (int j = 0; j < size; j++) {
        ...
        total +=
            (double)(src_char[start_pos + PIX(j, i, width) * bytes_per_pixel] |
                     src_char[start_pos + PIX(j, i, width) * bytes_per_pixel +
                              1]
                         << 8);
      }
    }
...
void apply_rgb_gain_offset_pre_debayer(struct device* dev, const void* p) {
  if (*r_gain == 256 && *g_gain == 256 && *b_gain == 256 && *r_offset == 0 &&
      *g_offset == 0 && *b_offset == 0)
    return;
 ...
 */
void enable_rgb_matrix(int red_red, int red_green, int red_blue, int green_red,
                       int green_green, int green_blue, int blue_red,
                       int blue_green, int blue_blue) {
  *rr = red_red;
  *rg = red_green;
  *rb = red_blue;
  *gr = green_red;
 ...
 * 		flag - set/reset the flag when get check button toggle
 */
void flip_enable(int enable) { enable_wrapper(flip_flag, enable); }
 ...
 * 		flag - set/reset the flag when get check button toggle
 */
void mirror_enable(int enable) { enable_wrapper(mirror_flag, enable); }
 ...
 * 		flag - set/reset the flag when get check button toggle
 */
void canny_filter_enable(int enable) { enable_wrapper(show_edge_flag, enable); }
 ...
 * 		flag - set/reset the flag when get check button toggle
 */
void histogram_enable(int enable) { enable_wrapper(histogram_ena, enable); }
 ...
 * 		flag - set/reset the flag when get check button toggle
 */
void separate_dual_enable(int enable) { enable_wrapper(separate_dual, enable); }
...
void add_alpha_val(int alpha_val_from_gui) { *alpha = alpha_val_from_gui; }

/** callback for set beta for brightness correction from gui */
void add_beta_val(int beta_val_from_gui) { *beta = beta_val_from_gui; }
 ...
 */
static void group_gpu_image_proc(cv::InputOutputArray opencvImage) {
  if (*(gamma_val) != (float)1) apply_gamma_correction(opencvImage, *gamma_val);

  if (*(alpha) != (float)1 || (*beta) != (float)0)
    apply_brightness_and_contrast(opencvImage, *alpha, *beta);

  if (*abc_flag) apply_auto_brightness_and_contrast(opencvImage, 1);

  if (*sharpness != (float)1) sharpness_control(opencvImage, *sharpness);

  if (*clahe_flag) apply_clahe(opencvImage);
...
void decode_process_a_frame(struct device* dev, const void* p,
  ...
  if (*flip_flag) cv::flip(share_img, share_img, 0);
  if (*mirror_flag) cv::flip(share_img, share_img, 1);
  if (*save_bmp) save_frame_image_bmp(share_img);
  ...
  if (*motion_detector_ena) motion_detector(share_img);
 ...
 * 		flag - set/reset the flag when get check button toggle
 */
void rgb_ir_ir_display_enable(int enable) { enable_wrapper(rgb_ir_ir, enable); }
...
int video_alloc_buffers(struct device* dev) {
  ...
  for (size_t i = 0; i < dev->nbufs; i++) {
    ...
    printf("length: %u offset: %u\n", querybuffer.length, querybuffer.m.offset);
